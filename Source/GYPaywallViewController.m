...
@interface GYPaywallViewController() <WKScriptMessageHandler, WKNavigationDelegate>//, WKUIDelegate>
...
+ (instancetype)paywallViewController:(GYPaywall*)p
{
    return [self paywallWithContent:p.content
                                url:p.contentUrl
                            version:p.version
                        paywallType:p.type
                               skus:p.skus
...
+ (instancetype)paywallWithContent:(NSString*)content
                               url:(NSURL*)url
                           version:(NSString*)version
                       paywallType:(GYPaywallType)paywallType
                              skus:(NSArray*)skus
                              pwid:(NSString*)pwid
...
//    [self.webview setUIDelegate:self];
    ...
    if (self.content) {
        [self.webview loadHTMLString:self.content baseURL:self.url];
    } else {
...
{
    NSDictionary *skusDetails;
    if (self.paywallType == GYPaywallTypeNoCode) {
        skusDetails = [self noCodeSkuDetails];
    } else {
        skusDetails = [self htmlSkuDetails];
    }
