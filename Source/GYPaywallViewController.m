...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    if ([action isEqualToString:@"close"]) {
        ...
        for (GYSku *s in self.skus) {
            if ([skuIdentifier isEqualToString:s.identifier]) {
                sku = s;
            }
        }
        if (!sku) {
...
- (NSDictionary *)skuDetails
{
    ...
    for (GYSku *s in self.skus) {
        ...
        NSDictionary *productEncoded = [p encodedObject];
        if (![productEncoded isKindOfClass:NSDictionary.class]) {
        ...
        skusDetail[@"product"] = productEncoded;
        skusDetail[@"msg"] = msg;
        skusDetail[@"identifier"] = s.identifier;
        ...
        skusDetails[s.identifier] = skusDetail;
    }
    ...
    for (int i = 0; i < self.skus.count; i++) {
        ...
        for (int j = 0; j < self.skus.count; j++) {
            ...
            skusDetails[sku.identifier][@"msg"][key] = [GYFormatter formatPercentage:@(discount) locale:self.locale];
        }
    }
